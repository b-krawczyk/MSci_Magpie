...
t,a,b,c,d,e,f,g,h = np.genfromtxt("Scope1.csv", delimiter=",", unpack=True)

fig1, (ax1, ax2, ax3) = plt.subplots(nrows=3, sharex=True)

ax1.grid()
ax1.plot(t[3500:5000], g[3500:5000])
ax1.plot(t[3500:5000], h[3500:5000])
ax1.set(ylabel="Rog Signal in V")
ax1.set(title="Main Return Post")


ax2.grid()
data_new = (-g[3500:5000]+h[3500:5000])/2
...
f_fft = fft(g[3500:5000])
e_fft = fft(h[3500:5000])
...
data = [t_new, np.abs(np.array(data_integrated))]
np.savetxt("target_current.csv", data, delimiter=",")

plt.xlabel("time in ns")
ax6.set(ylabel="Current in MA")

print(np.abs(sum(data_new))*len(t_new)*(t_new[1]-t_new[0])/1000000000*3*10**9*8*170)
print(sum((np.array(h[3500:5000])-np.array(g[3500:5000]))/2)*len(t[3500:5000])*(t[3501]-t[3500])/1000000000*3*10**9*8*170)
print(len(t_new))
